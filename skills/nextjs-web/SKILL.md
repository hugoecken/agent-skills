...
- Next.js owns rendering, not product authorization, OAuth session ownership, direct database access or another business API. Do not add product-mutating Server Actions.
...
Derive canonical content and metadata from the same public source. Keep canonical anonymous HTML independent of the user's session; fetch optional account controls through a private endpoint after hydration when the local architecture requires it. Preserve product-defined visibility, publication and discovery semantics rather than inferring them from route names.
...
Use generated Orval client functions and types. Validate consumed JSON at the transport boundary before successful caching. Keep view/form models only for genuinely different editing or display semantics. New interactive forms use React Hook Form and Zod; existing form-stack migration remains a separate accepted task. Product mutations call the generated BFF client.

Use effects only for external synchronization with dependencies/cleanup, never to derive render state or handle ordinary events. Keep simple query/local-state ownership in the screen; no obligatory screen-hook layer.
