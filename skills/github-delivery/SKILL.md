...
Build an authorized prototype as a separate project outside the application repository. Review it directly, using screenshots when useful; do not create an application branch, PR or artificial tracking issue for this exploration. Keep the project available for further review and remove it only on explicit request. If an existing issue owns the exploration, attach the accepted evidence there. Prototype approval does not bypass the Figma gate or turn its code into production implementation.
